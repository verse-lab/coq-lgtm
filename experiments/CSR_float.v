...
From LGTM.experiments Require Import Prelude UnaryCommon SV_float.
...
Proof with (try solve [ seclocal_solver ]; seclocal_fold).
  ...
  xin 2 : do 3 (xwp; xapp).
  xin 1 : (xwp; xapp (@htriple_allocf0_unary)=> // s)...
  rewrite prod_cascade.
...
Proof with (try seclocal_fold; seclocal_solver).
  ...
  { xin (2,0) : rewrite wp_prod_single /=.
    ...
    xfor_sum Inv2 R2 R2 (fun hv i => (to_float (hv[`2](colind[i])), dvec[colind[i] ])) tmp.
    { rewrite /Inv2 /R2.
      xin 1 : xwp; xapp(@lhtriple_array_read_float); xwp; xapp(@lhtriple_array_read); xwp; xapp(@lhtriple_array_read_float). (* TODO why simply xapp does not work here? *)
      (xin 1: xapp (@fma.spec)=> y)... 
