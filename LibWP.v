...
(* Module HD : Domain with Definition type := labeled D.
Definition type := labeled D.
End HD.

Module NR := Reasoning(HD).
Export NR. *)
(*
Check eq_refl : D = labeled D.
Check eq_refl : HD = labeled D.
*)
...
Lemma xfor_lemma2
  Inv 
  (R1 R2 R1' R2' : D -> hhprop)
  m (H : int -> hhprop) (H' : int -> (D -> val) -> hhprop)
  s fsi1 fsi2 vr
  Z N (i j k : nat) (C1 C2 : D -> trm) (C : trm)
  Pre Post: 
  (forall (l : int) Q, 
    Z <= l <= N ->
    {{ Inv l \* 
