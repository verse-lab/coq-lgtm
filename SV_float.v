...
From SLF Require Import LabType Fun LibSepFmap Sum Unary Dotprod_float Prelude.
From SLF Require Import LibWP LibSepSimpl LibSepReference LibSepTLCbuffer Struct Struct2 Loops Loops2 ListCommon.
...
Proof with (try solve [ seclocal_solver | auto using finite_suffcond ]; fold').
  ...
  rewrite intr_list ?(fset_of_list_nodup 0) ?Hl ?Union_interval_change2 //.
  xfor_sum Inv R R (fun hv i => (to_float (hv[`2](xind[i])), dvec[xind[i] ])) q...
