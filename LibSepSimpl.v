...
Parameter hhprop : Type.

Parameter himpl : hhprop -> hhprop -> Prop.

Definition qimpl A (Q1 Q2:A->hhprop) :=
  forall r, himpl (Q1 r) (Q2 r).

(** The core operators of Separation Logic must be provided. *)

Parameter hempty : hhprop.

Parameter hstar : hhprop -> hhprop -> hhprop.

Parameter hpure : Prop -> hhprop.

Parameter htop : hhprop.

Parameter hgc : hhprop.

Parameter hwand : hhprop -> hhprop -> hhprop.

Parameter qwand : forall A, (A->hhprop) -> (A->hhprop) -> hhprop.

Parameter hexists : forall A, (A->hhprop) -> hhprop.

Parameter hforall : forall A, (A->hhprop) -> hhprop.
...
Parameter himpl_hexists_l : forall A H (J:A->hhprop),
  (forall x, J x ==> H) ->
  (hexists J) ==> H.
...
Parameter hstar_hexists : forall A (J:A->hhprop) H,
  (hexists J) \* H = hexists (fun x => (J x) \* H).

(** Characterization of [hforall] *)

Parameter himpl_hforall_r : forall A (J:A->hhprop) H,
  (forall x, H ==> J x) ->
  H ==> (hforall J).

Parameter hstar_hforall : forall H A (J:A->hhprop),
...
Parameter qwand_equiv : forall H A (Q1 Q2:A->hhprop),
  H ==> (Q1 \--* Q2) <-> (Q1 \*+ H) ===> Q2.

Parameter hwand_cancel : forall H1 H2,
  H1 \* (H1 \-* H2) ==> H2.

Parameter qwand_specialize : forall A (x:A) (Q1 Q2:A->hhprop),
  (Q1 \--* Q2) ==> (Q1 x \-* Q2 x).
...
Lemma qimpl_trans : forall A (Q2 Q1 Q3:A->hhprop),
  (Q1 ===> Q2) ->
  (Q2 ===> Q3) ->
  (Q1 ===> Q3).
Proof using. introv M1 M2. intros v. applys* himpl_trans. Qed.

Lemma qimpl_antisym : forall A (Q1 Q2:A->hhprop),
  (Q1 ===> Q2) ->
  (Q2 ===> Q1) ->
  (Q1 = Q2).
...
Definition repr (A:Type) (S:A->hhprop) (x:A) : hhprop :=
  S x.

Notation "x '~>' S" := (repr S x)
  (at level 33, no associativity) : heap_scope.

Lemma repr_eq : forall (A:Type) (S:A->hhprop) (x:A),
  (x ~> S) = (S x).
...
Lemma star_post_empty : forall B (Q:B->hhprop),
  Q \*+ \[] = Q.
...
Definition Xsimpl (HL HR:hhprop*hhprop*hhprop) :=
...
Ltac xsimpl_l_start M :=
  introv M;
  match goal with HR: hhprop*hhprop*hhprop |- _ =>
...
Lemma xsimpl_l_hexists : forall A (J:A->hhprop) Hla Hlw Hlt HR,
...
Lemma xsimpl_l_cancel_qwand : forall A (x:A) (Q1 Q2:A->hhprop) Hla Hlw Hlt HR,
...
Ltac xsimpl_r_start M :=
  introv M;
  match goal with HL: hhprop*hhprop*hhprop |- _ =>
...
Lemma xsimpl_r_hexists : forall A (x:A) (J:A->hhprop) Hra Hrg Hrt HL,
...
Lemma xsimpl_lr_cancel_eq_repr : forall A p (E1 E2:A->hhprop) Hla Hlw Hlt Hra Hrg Hrt,
  E1 = E2 ->
  Xsimpl (Hla, Hlw, Hlt) (Hra, Hrg, Hrt) ->
...
Lemma xsimpl_lr_qwand : forall A (Q1 Q2:A->hhprop) Hla,
...
Lemma xsimpl_lr_qwand_unit : forall (Q1 Q2:unit->hhprop) Hla,
  Xsimpl (\[], \[], (Q1 tt \* Hla)) (\[], \[], (Q2 tt \* \[])) ->
  Xsimpl (Hla, \[], \[]) ((Q1 \--* Q2) \* \[], \[], \[]).
Proof using. introv M. applys xsimpl_lr_qwand. intros []. applys M. Qed.

Lemma xsimpl_lr_hforall : forall A (J:A->hhprop) Hla,
...
Lemma himpl_lr_qwand_unify : forall A (Q:A->hhprop) Hla,
  Xsimpl (Hla, \[], \[]) ((Q \--* (Q \*+ Hla)) \* \[], \[], \[]).
...
Ltac xsimpl_handle_qimpl tt :=
  ...
  | |- himpl _ _ => idtac
  | |- @eq hhprop _ _ => applys himpl_antisym
  | |- @eq (_ -> hhprop) _ _ => applys fun_ext_1; applys himpl_antisym
...
Lemma xsimpl_pick_demo : forall (Q:bool->hhprop) (P:Prop) H1 H2 H3 Hlw Hlt Hra Hrg Hrt,
  (forall HX HY,
...
Lemma xsimpl_demo_hint : forall H1 (Q:int->hhprop),
  Q 4 ==> Q 3 ->
...
Lemma xsimpl_demo_qwand : forall A (x:A) (Q1 Q2:A->hhprop) H1,
...
Lemma xsimpl_demo_qwand_r : forall A (x:A) (Q1 Q2:A->hhprop) H1 H2,
  H1 \* H2 ==> H1 \* (Q1 \--* (Q1 \*+ H2)).
Proof using. intros. xsimpl. Qed.
...
Lemma xsimpl_demo_repr_1 : forall p q (R:int->int->hhprop),
...
Lemma xsimpl_demo_repr_2 : forall p (R R':int->int->hhprop),
  R = R' ->
  p ~> R' 3 ==> \exists n, p ~> R n.
Proof using. introv E. xsimpl. subst R'. xsimpl. Qed.

Lemma xsimpl_demo_repr_3 : forall p (R:int->int->hhprop),
  let R' := R in
  p ~> R' 3 ==> \exists n, p ~> R n.
Proof using.
  intros. dup.
...
Lemma xsimpl_demo_repr_4 : forall p n m (R:int->int->hhprop),
  n = m + 0 ->
  p ~> R n ==> p ~> R m.
...
Lemma xchange_demo_2 : forall A (Q:A->hhprop) H1 H2 H3,
...
Lemma xchange_demo_4 : forall A (Q1 Q2:A->hhprop) H,
  Q1 ===> Q2 ->
  Q1 \*+ H ===> Q2 \*+ H.
...
Lemma xchange_demo_hforall_l :
 forall (hforall_specialize : forall A (x:A) (J:A->hhprop), (hforall J) ==> (J x)),
 forall (Q:int->hhprop) H1,
