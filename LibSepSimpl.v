...
(* Module  (HP : XsimplParams). *)

Open Scope hprop_scope.

(* Implicit Types H : @hhprop D. *)
(* Implicit Types Q : val -> @hhprop D. *)
...
Arguments hhprop : clear implicits.

(* ================================================================= *)
(** ** Properties of [himpl] *)

Lemma himpl_of_eq {D : Type} : forall (H1 H2 : hhprop D),
  H1 = H2 ->
  H1 ==> H2.
Proof. intros. subst. applys~ @himpl_refl. Qed.

Lemma himpl_of_eq_sym {D : Type} : forall (H1 H2 : hhprop D),
  H1 = H2 ->
  H2 ==> H1.
Proof. intros. subst. applys~ @himpl_refl. Qed.
...
Lemma qimpl_refl{D : Type} : forall A (Q:A->hhprop D),
  Q ===> Q.
Proof using. intros. hnfs*. Qed.

Global Hint Resolve qimpl_refl.

Lemma qimpl_trans {D : Type} : forall A (Q2 Q1 Q3:A->hhprop D),
  (Q1 ===> Q2) ->
  (Q2 ===> Q3) ->
  (Q1 ===> Q3).
Proof using. introv M1 M2. intros v. applys* @himpl_trans. Qed.

Lemma qimpl_antisym {D : Type} : forall A (Q1 Q2:A->hhprop D),
  (Q1 ===> Q2) ->
  (Q2 ===> Q1) ->
  (Q1 = Q2).
...
Lemma hstar_comm_assoc {D : Type} : forall (H1 H2 H3 : hhprop D),
  H1 \* H2 \* H3 = H2 \* H1 \* H3.
Proof using.
  intros. rewrite <- hstar_assoc.
  rewrite (@hstar_comm _ H1 H2). rewrite~ @hstar_assoc.
Qed.
...
Definition repr {D : Type} (A:Type) (S:A->hhprop D) (x:A) : hhprop D :=
  S x.
...
Lemma repr_eq {D : Type} : forall (A:Type) (S:A->hhprop D) (x:A),
  (x ~> S) = (S x).
...
Definition Id {D : Type} {A:Type} (X:A) (x:A) : hhprop D :=
  \[ X = x ].
...
Lemma repr_id : forall {D : Type} A (x X:A),
  (x ~> Id X) = \[X = x] :> hhprop D.
...
Lemma star_post_empty {D : Type} : forall B (Q:B->hhprop D),
  Q \*+ \[] = Q.
...
Ltac remove_empty_heaps_from H :=
  match H with context[ ?H1 \* \[] ] =>
    ...
    | false => rewrite (@hstar_hempty_r _ H1)
    | true => let X := fresh in
              set (X := H1);
              rewrite (@hstar_hempty_r _ X);
              subst X
...
Lemma hstars_pick_9 {D : Type} : forall (H1 H2 H3 H4 H5 H6 H7 H8 H9 H : hhprop D),
...
Lemma hstars_pick_last_2 {D : Type} : forall (H1 H2 : hhprop D),
  H1 \* H2 = H2 \* H1.
Proof using. intros. rewrite~ (@hstar_comm). Qed.

Lemma hstars_pick_last_3 {D : Type} : forall (H1 H2 H3 : hhprop D),
  H1 \* H2 \* H3 = H3 \* H1 \* H2.
Proof using. intros. rewrite~ (hstar_comm H2). applys @hstars_pick_2. Qed.

Lemma hstars_pick_last_4 {D : Type} : forall (H1 H2 H3 H4 : hhprop D),
  H1 \* H2 \* H3 \* H4 = H4 \* H1 \* H2 \* H3.
Proof using. intros. rewrite~ (hstar_comm H3). applys @hstars_pick_3. Qed.

Lemma hstars_pick_last_5 {D : Type} : forall (H1 H2 H3 H4 H5 : hhprop D),
  H1 \* H2 \* H3 \* H4 \* H5 = H5 \* H1 \* H2 \* H3 \* H4.
Proof using. intros. rewrite~ (hstar_comm H4). applys @hstars_pick_4. Qed.

Lemma hstars_pick_last_6 {D : Type} : forall (H1 H2 H3 H4 H5 H6 : hhprop D),
    H1 \* H2 \* H3 \* H4 \* H5 \* H6
  = H6 \* H1 \* H2 \* H3 \* H4 \* H5.
Proof using. intros. rewrite~ (hstar_comm H5). applys @hstars_pick_5. Qed.

Lemma hstars_pick_last_7 {D : Type} : forall (H1 H2 H3 H4 H5 H6 H7 : hhprop D),
    H1 \* H2 \* H3 \* H4 \* H5 \* H6 \* H7
  = H7 \* H1 \* H2 \* H3 \* H4 \* H5 \* H6.
Proof using. intros. rewrite~ (hstar_comm H6). applys @hstars_pick_6. Qed.

Lemma hstars_pick_last_8 {D : Type} : forall (H1 H2 H3 H4 H5 H6 H7 H8 : hhprop D),
    H1 \* H2 \* H3 \* H4 \* H5 \* H6 \* H7 \* H8
  = H8 \* H1 \* H2 \* H3 \* H4 \* H5 \* H6 \* H7.
Proof using. intros. rewrite~ (hstar_comm H7). applys @hstars_pick_7. Qed.

Lemma hstars_pick_last_9 {D : Type} : forall (H1 H2 H3 H4 H5 H6 H7 H8 H9 : hhprop D),
    H1 \* H2 \* H3 \* H4 \* H5 \* H6 \* H7 \* H8 \* H9
...
Definition Xsimpl {D : Type} (HL HR:@hhprop D *@hhprop D*@hhprop D) :=
...
Lemma Xsimpl_trans_l {D : Type} : forall Hla1 Hlw1 Hlt1 Hla2 Hlw2 Hlt2 HR,
  Xsimpl (Hla2,Hlw2,Hlt2) HR ->
  Hla1 \* Hlw1 \* Hlt1 ==> Hla2 \* Hlw2 \* Hlt2 ->
  @Xsimpl D (Hla1,Hlw1,Hlt1) HR.
Proof using.
  introv M1 E. destruct HR as [[Hra Hrg] Hrt]. unfolds @Xsimpl.
  applys* @himpl_trans M1.
Qed.

Lemma Xsimpl_trans_r {D : Type} : forall Hra1 Hrg1 Hrt1 Hra2 Hrg2 Hrt2 HL,
  Xsimpl HL (Hra2,Hrg2,Hrt2) ->
  Hra2 \* Hrg2 \* Hrt2 ==> Hra1 \* Hrg1 \* Hrt1 ->
  @Xsimpl D HL (Hra1,Hrg1,Hrt1).
Proof using.
  introv M1 E. destruct HL as [[Hla Hlw] Hlt]. unfolds @Xsimpl.
  applys* @himpl_trans M1.
Qed.

Lemma Xsimpl_trans {D : Type} : forall 
  (Hla1 Hlw1 Hlt1 Hla2 Hlw2 Hlt2 Hra1 Hrg1 Hrt1 Hra2 Hrg2 Hrt2 : @hhprop D),
...
Lemma hstars_simpl_start {D : Type} : forall (H1 H2 : @hhprop D),
  H1 \* \[] ==> \[] \* H2 \* \[] ->
  H1 ==> H2.
Proof using. introv M. rew_heap~ in *. Qed.

Lemma hstars_simpl_keep {D : Type} : forall (H1 Ha H Ht : hhprop D),
  H1 ==> (H \* Ha) \* Ht ->
  H1 ==> Ha \* H \* Ht.
Proof using. introv M. rew_heap in *. rewrite~ @hstar_comm_assoc. Qed.

Lemma hstars_simpl_cancel {D : Type} : forall (H1 Ha H Ht : hhprop D),
  H1 ==> Ha \* Ht ->
  H \* H1 ==> Ha \* H \* Ht.
Proof using. introv M. rewrite hstar_comm_assoc. applys~ @himpl_frame_lr. Qed.

Lemma hstars_simpl_pick_lemma {D : Type} : forall (H1 H1' H2 : hhprop D),
  H1 = H1' ->
  H1' ==> H2 ->
  H1 ==> H2.
...
Lemma xpull_protect {D : Type} : forall (H1 H2 : hhprop D),
  H1 ==> protect H2 ->
  H1 ==> H2.
Proof using. auto. Qed.

Lemma xsimpl_start {D : Type} : forall (H1 H2 : hhprop D),
  Xsimpl (\[], \[], (H1 \* \[])) (\[], \[], (H2 \* \[])) ->
  H1 ==> H2.
...
Ltac xsimpl_l_start M :=
  introv M;
  match goal with HR: (@hhprop _) * (@hhprop _) * (@hhprop _) |- _ =>
    destruct HR as [[Hra Hrg] Hrt]; unfolds @Xsimpl end.

Ltac xsimpl_l_start' M :=
  xsimpl_l_start M; applys @himpl_trans (rm M); hstars_simpl.

Lemma xsimpl_l_hempty {D : Type} : forall (Hla Hlw Hlt : hhprop D) HR,
  Xsimpl (Hla, Hlw, Hlt) HR ->
  Xsimpl (Hla, Hlw, (\[] \* Hlt)) HR.
Proof using. xsimpl_l_start' M. Qed.

Lemma xsimpl_l_hpure {D : Type} : forall P (Hla Hlw Hlt : hhprop D) HR,
...
Lemma xsimpl_l_hexists {D : Type} : forall A (J:A->hhprop D) (Hla Hlw Hlt : hhprop D) HR,
...
Lemma xsimpl_l_acc_wand {D : Type} : forall (H Hla Hlw Hlt : hhprop D) HR,
  Xsimpl (Hla, (H \* Hlw), Hlt) HR ->
  Xsimpl (Hla, Hlw, (H \* Hlt)) HR.
Proof using. xsimpl_l_start' M. Qed.

Lemma xsimpl_l_acc_other {D : Type} : forall (H Hla Hlw Hlt : hhprop D) HR,
...
Lemma xsimpl_l_cancel_hwand_hempty {D : Type} : forall (H2 Hla Hlw Hlt : hhprop D) HR,
  Xsimpl (Hla, Hlw, (H2 \* Hlt)) HR ->
  Xsimpl (Hla, ((\[] \-* H2) \* Hlw), Hlt) HR.
Proof using. xsimpl_l_start' M. Qed.

Lemma xsimpl_l_cancel_hwand {D : Type} : forall (H1 H2 Hla Hlw Hlt : hhprop D) HR,
  Xsimpl (\[], Hlw, (Hla \* H2 \* Hlt)) HR ->
  Xsimpl ((H1 \* Hla), ((H1 \-* H2) \* Hlw), Hlt) HR.
Proof using. xsimpl_l_start' M. applys~ @hwand_cancel. Qed.

Lemma xsimpl_l_cancel_qwand {D : Type} : forall A (x:A) (Q1 Q2:A->hhprop D) (Hla Hlw Hlt : hhprop D) HR,
...
Lemma xsimpl_l_keep_wand {D : Type} : forall (H Hla Hlw Hlt : hhprop D) HR,
  Xsimpl ((H \* Hla), Hlw, Hlt) HR ->
  Xsimpl (Hla, (H \* Hlw), Hlt) HR.
Proof using. xsimpl_l_start' M. Qed.

Lemma xsimpl_l_hwand_reorder {D : Type} : forall (H1 H1' H2 Hla Hlw Hlt : hhprop D) HR,
  H1 = H1' ->
  Xsimpl (Hla, ((H1' \-* H2) \* Hlw), Hlt) HR ->
  Xsimpl (Hla, ((H1 \-* H2) \* Hlw), Hlt) HR.
Proof using. intros. subst*. Qed.

Lemma xsimpl_l_cancel_hwand_hstar {D : Type} : forall (H1 H2 H3 Hla Hlw Hlt : hhprop D) HR,
...
Lemma xsimpl_l_cancel_hwand_hstar_hempty {D : Type} : forall (H2 H3 Hla Hlw Hlt : hhprop D) HR,
  Xsimpl (Hla, Hlw, ((H2 \-* H3) \* Hlt)) HR ->
...
Lemma xsimpl_r_hexists {D : Type} : forall A (x:A) (J:A->hhprop D) (Hra Hrg Hrt : hhprop D) HL,
  Xsimpl HL (Hra, Hrg, (J x \* Hrt)) ->
  Xsimpl HL (Hra, Hrg, (hexists J \* Hrt)).
Proof using. xsimpl_r_start' M. applys* @himpl_hexists_r. Qed.

Lemma xsimpl_r_id {D : Type} : forall A (x X:A) (Hra Hrg Hrt : hhprop D) HL,
  (X = x) ->
  Xsimpl HL (Hra, Hrg, Hrt) ->
...
Lemma xsimpl_r_id_unify {D : Type} : forall A (x:A) (Hra Hrg Hrt : hhprop D) HL,
  Xsimpl HL (Hra, Hrg, Hrt) ->
  Xsimpl HL (Hra, Hrg, (x ~> Id x \* Hrt)).
Proof using. introv M. applys~ @xsimpl_r_id. Qed.

Lemma xsimpl_r_keep {D : Type} : forall (H Hra Hrg Hrt : hhprop D) HL,
...
Lemma xsimpl_r_hgc_or_htop {D : Type} : forall (H Hra Hrg Hrt : hhprop D) HL,
  Xsimpl HL (Hra, (H \* Hrg), Hrt) ->
  Xsimpl HL (Hra, Hrg, (H \* Hrt)).
Proof using. xsimpl_r_start' M. Qed.

Lemma xsimpl_r_htop_replace_hgc {D : Type} : forall (Hra Hrg Hrt : hhprop D) HL,
  Xsimpl HL (Hra, (\Top \* Hrg), Hrt) ->
  Xsimpl HL (Hra, (\GC \* Hrg), (\Top \* Hrt)).
Proof using. xsimpl_r_start' M. applys @himpl_hgc_r. xaffine. Qed.

Lemma xsimpl_r_hgc_drop {D : Type} : forall (Hra Hrg Hrt : hhprop D) HL,
  Xsimpl HL (Hra, Hrg, Hrt) ->
  Xsimpl HL (Hra, Hrg, (\GC \* Hrt)).
Proof using. xsimpl_r_start' M. applys @himpl_hgc_r. xaffine. Qed.

Lemma xsimpl_r_htop_drop {D : Type} : forall (Hra Hrg Hrt : hhprop D) HL,
  Xsimpl HL (Hra, Hrg, Hrt) ->
...
Lemma xsimpl_lr_cancel_same {D : Type} : forall (H Hla Hlw Hlt Hra Hrg Hrt : hhprop D),
  Xsimpl (Hla, Hlw, Hlt) (Hra, Hrg, Hrt) ->
  Xsimpl ((H \* Hla), Hlw, Hlt) (Hra, Hrg, (H \* Hrt)).
Proof using. xsimpl_lr_start' M. Qed.

Lemma xsimpl_lr_cancel_htop {D : Type} : forall (H Hla Hlw Hlt Hra Hrg Hrt : hhprop D),
...
Lemma xsimpl_lr_cancel_hgc {D : Type} : forall (Hla Hlw Hlt Hra Hrg Hrt : hhprop D),
...
Lemma xsimpl_lr_cancel_eq {D : Type} : forall (H1 H2 Hla Hlw Hlt Hra Hrg Hrt : hhprop D),
  (H1 = H2) ->
  Xsimpl (Hla, Hlw, Hlt) (Hra, Hrg, Hrt) ->
  Xsimpl ((H1 \* Hla), Hlw, Hlt) (Hra, Hrg, (H2 \* Hrt)).
Proof using. introv ->. apply~ @xsimpl_lr_cancel_same. Qed.

Lemma xsimpl_lr_cancel_eq_repr {D : Type} : forall A p (E1 E2:A->hhprop D) (Hla Hlw Hlt Hra Hrg Hrt : hhprop D),
  E1 = E2 ->
  Xsimpl (Hla, Hlw, Hlt) (Hra, Hrg, Hrt) ->
  Xsimpl (((p ~> E1) \* Hla), Hlw, Hlt) (Hra, Hrg, ((p ~> E2) \* Hrt)).
Proof using. introv M. subst. apply~ @xsimpl_lr_cancel_same. Qed.

Lemma xsimpl_lr_hwand {D : Type} : forall (H1 H2 Hla : hhprop D),
  Xsimpl (\[], \[], (H1 \* Hla)) (\[], \[], H2 \* \[]) ->
  Xsimpl (Hla, \[], \[]) ((H1 \-* H2) \* \[], \[], \[]).
Proof using.
...
Lemma xsimpl_lr_hwand_hfalse {D : Type} : forall (Hla H1 : hhprop D),
  Xsimpl (Hla, \[], \[]) ((\[False] \-* H1) \* \[], \[], \[]).
Proof using.
...
Lemma xsimpl_lr_qwand {D : Type} : forall A (Q1 Q2:A->hhprop D) (Hla : hhprop D),
...
Lemma xsimpl_lr_qwand_unit {D : Type} : forall (Q1 Q2:unit->hhprop D) (Hla : hhprop D),
  Xsimpl (\[], \[], (Q1 tt \* Hla)) (\[], \[], (Q2 tt \* \[])) ->
  Xsimpl (Hla, \[], \[]) ((Q1 \--* Q2) \* \[], \[], \[]).
Proof using. introv M. applys @xsimpl_lr_qwand. intros []. applys @M. Qed.

Lemma xsimpl_lr_hforall {D : Type} : forall A (J:A->hhprop D) Hla,
...
Lemma himpl_lr_refl {D : Type} : forall (Hla : hhprop D),
  Xsimpl (Hla, \[], \[]) (Hla, \[], \[]).
Proof using. intros. unfolds @Xsimpl. hstars_simpl. Qed.

Lemma himpl_lr_qwand_unify {D : Type} : forall A (Q:A->hhprop D) Hla,
  Xsimpl (Hla, \[], \[]) ((Q \--* (Q \*+ Hla)) \* \[], \[], \[]).
Proof using. intros. unfolds @Xsimpl. hstars_simpl. rewrite~ @qwand_equiv. Qed.

Lemma himpl_lr_htop {D : Type} : forall (Hla Hrg : hhprop D),
  Xsimpl (\[], \[], \[]) (\[], Hrg, \[]) ->
  Xsimpl (Hla, \[], \[]) (\[], (\Top \* Hrg), \[]).
Proof using.
...
Lemma himpl_lr_hgc {D : Type} : forall (Hla Hrg : hhprop D),
  haffine Hla ->
  Xsimpl (\[], \[], \[]) (\[], Hrg, \[]) ->
  Xsimpl (Hla, \[], \[]) (\[], (\GC \* Hrg), \[]).
Proof using.
...
Lemma xsimpl_lr_exit_nogc {D : Type} : forall (Hla Hra : hhprop D),
  Hla ==> Hra ->
  Xsimpl (Hla, \[], \[]) (Hra, \[], \[]).
Proof using. introv M. unfolds @Xsimpl. hstars_simpl. auto. Qed.

Lemma xsimpl_lr_exit {D : Type} : forall (Hla Hra Hrg : hhprop D),
  Hla ==> Hra \* Hrg ->
  Xsimpl (Hla, \[], \[]) (Hra, Hrg, \[]).
...
Lemma xsimpl_flip_acc_l {D : Type} : forall (Hla Hra Hla' Hrg : hhprop D),
  Hla = Hla' ->
  Xsimpl (Hla', \[], \[]) (Hra, Hrg, \[]) ->
  Xsimpl (Hla, \[], \[]) (Hra, Hrg, \[]).
Proof using. introv E1 M. subst*. Qed.

Lemma xsimpl_flip_acc_r {D : Type} : forall (Hla Hra Hra' Hrg : hhprop D),
  Hra = Hra' ->
  Xsimpl (Hla, \[], \[]) (Hra', Hrg, \[]) ->
  Xsimpl (Hla, \[], \[]) (Hra, Hrg, \[]).
Proof using. introv E1 M. subst*. Qed.
...
(** [xsimpl_pick i] applies to a goal of the form
 ...
 Lemma xsimpl_pick_lemma {D : Type} : forall (Hla1 Hla2 Hlw Hlt : hhprop D) HR,
  Hla1 = Hla2 ->
...
Ltac xsimpl_pick_st f :=
  match goal with |- @Xsimpl _ (?Hla, ?Hlw, ?Hlt) ?HR =>
...
Ltac xsimpl_handle_qimpl tt :=
  ...
  | |- @qimpl _ unit ?Q1 ?Q2 => let t := fresh "_tt" in intros t; destruct t
  | |- @qimpl _ _ _ _ => let r := fresh "r" in intros r
  | |- @himpl _ _ ?H2 => is_evar H2; apply himpl_refl
  | |- @himpl _ _ _ => idtac
  | |- @eq (@hhprop _) _ _ => applys @himpl_antisym
  | |- @eq (_ -> @hhprop _) _ _ => applys @fun_ext_1; applys @himpl_antisym
...
Ltac xsimpl_r_hexists_apply tt :=
  first [
    xsimpl_hint_next ltac:(fun x =>
      ...
      | _ => apply (@xsimpl_r_hexists _ _ x)
      end)
...
Ltac xsimpl_hwand_hstars_l tt :=
  match goal with |- @Xsimpl _ (?Hla, ((?H1s \-* ?H2) \* ?Hlw), \[]) ?HR =>
...
Ltac xsimpl_step_l tt :=
  match goal with |- @Xsimpl _ ?HL ?HR =>
  match HL with
  | (?Hla, ?Hlw, (?H \* ?Hlt)) =>
    match H with
    | @hempty _ => apply xsimpl_l_hempty
    | @hpure _ ?P => apply xsimpl_l_hpure; intro
    | @hstar _ ?H1 ?H2 => rewrite (@hstar_assoc _ H1 H2)
    | @hexists _ _ ?J => apply xsimpl_l_hexists; intro
...
Ltac xsimpl_hgc_or_htop_step tt :=
  match goal with |- @Xsimpl _ (?Hla, \[], \[]) (?Hra, ?Hrg, (?H \* ?Hrt)) =>
    match constr:((Hrg,H)) with
    | (\[], @hgc ?D) => applys @xsimpl_r_hgc_or_htop;
                    xsimpl_hgc_or_htop_cancel (@hgc D) xsimpl_lr_cancel_hgc
    | (\[], @htop ?D) => applys @xsimpl_r_hgc_or_htop;
                    xsimpl_hgc_or_htop_cancel (@hgc D) xsimpl_lr_cancel_htop;
                    xsimpl_hgc_or_htop_cancel (@htop D) xsimpl_lr_cancel_htop
...
Ltac xsimpl_step_r tt :=
  ...
  | ?H1 \* ?H2 => rewrite (@hstar_assoc _ H1 H2)
  | ?H \-* ?H'eqH =>
      match H with
  ...
  | @hexists _ _ ?J => xsimpl_r_hexists_apply tt
  | \GC => xsimpl_hgc_or_htop_step tt
  | @htop _ => xsimpl_hgc_or_htop_step tt
...
Ltac xsimpl_step_lr tt :=
  match goal with |- @Xsimpl _ (?Hla, \[], \[]) (?Hra, ?Hrg, \[]) =>
    match Hrg with
    | \[] =>
       match Hra with
       | ?H1 \* \[] =>
...
Lemma xchange_lemma {D : Type} : forall (H1 H2 H3 H4 : hhprop D),
  H1 ==> H2 ->
  H3 ==> H1 \* (H2 \-* protect H4) ->
  H3 ==> H4.
Proof using.
...
(** *** [rew_heap] demos *)
 ...
 Lemma rew_heap_demo_with_evar {D : Type} : forall (H1 H2 H3 : hhprop D),
...
Lemma hstars_flip_demo {D : Type} : forall (H1 H2 H3 H4 : hhprop D),
  (forall H, H1 \* H2 \* H3 \* H4 \* \[] = H -> H = H -> True) -> True.
Proof using.
  introv M. eapply M. hstars_flip tt.
Abort.

Lemma hstars_flip_demo_0 {D : Type} :
  (forall H, \[] = H :> hhprop D -> H = H -> True) -> True.
Proof using.
  introv M. eapply M. hstars_flip tt.
Abort.
...
Lemma demo_hstars_pick_1 {D : Type} : forall (H1 H2 H3 H4 Hresult : hhprop D),
...
(** *** [hstars_simpl] demos *)
 ...
 Lemma demo_hstars_simpl_1 {D : Type} : forall (H1 H2 H3 H4 H5 : hhprop D),
  H2 ==> H5 ->
...
Lemma demo_hstars_simpl_2 {D : Type}: forall (H1 H2 H3 H4 H5 : hhprop D),
...
Lemma xsimpl_pick_demo {D : Type} : forall (Q:bool->hhprop D) (P:Prop) (H1 H2 H3 Hlw Hlt Hra Hrg Hrt : hhprop D),
  (forall HX HY,
...
Proof using.
  ...
  xsimpl_pick_unifiable (\[True] : hhprop D).
...
Lemma xpull_demo {D : Type} : forall (H1 H2 H3 H : hhprop D),
...
Lemma xsimpl_demo_stars {D : Type} : forall (H1 H2 H3 H4 H5 : hhprop D),
...
Lemma xsimpl_demo_keep_order  {D : Type} : forall (H1 H2 H3 H4 H5 H6 H7 : hhprop D),
  H1 \* H2 \* H3 \* H4 ==> H5 \* H3 \* H6 \* H7.
Proof using. intros. xsimpl. demo. Abort.

Lemma xsimpl_demo_stars_top {D : Type} : forall (H1 H2 H3 H4 H5 : hhprop D),
...
Lemma xsimpl_demo_hint {D : Type} : forall H1 (Q:int->hhprop D),
  Q 4 ==> Q 3 ->
...
Lemma xsimpl_demo_evar_1 {D : Type} : forall (H1 H2 : hhprop D),
  (forall H, H1 \* H2 ==> H -> True) -> True.
Proof using. intros. eapply H. xsimpl. Abort.

Lemma xsimpl_demo_evar_2 {D : Type} : forall (H1 : hhprop D),
...
Lemma xsimpl_demo_htop_both_sides {D : Type} : forall (H1 H2 : hhprop D),
  H1 \* H2 \* \Top ==> H1 \* \Top.
Proof using.
  dup.
  { intros. xsimpl0. xsimpl1. xsimpl1. xsimpl1. xsimpl1. xsimpl1.
...
Lemma xsimpl_demo_htop_multiple {D : Type} : forall (H1 H2 : hhprop D),
...
Lemma xsimpl_demo_hwand {D : Type} : forall (H1 H2 H3 H4 : hhprop D),
...
Lemma xsimpl_demo_qwand {D : Type} : forall A (x:A) (Q1 Q2:A->hhprop D) H1,
  H1 \* (H1 \-* (Q1 \--* Q2)) \* (Q1 x) ==> (Q2 x).
Proof using. intros. xsimpl~. Qed.

Lemma xsimpl_demo_hwand_r {D : Type} : forall (H1 H2 H3 : hhprop D),
  H1 \* H2 ==> H1 \* (H3 \-* (H2 \* H3)).
Proof using. intros. xsimpl~. Qed.

Lemma xsimpl_demo_qwand_r {D : Type} : forall A (x:A) (Q1 Q2:A->hhprop D) (H1 H2 : hhprop D),
  H1 \* H2 ==> H1 \* (Q1 \--* (Q1 \*+ H2)).
Proof using. intros. xsimpl. Qed.

Lemma xsimpl_demo_hwand_multiple_1 {D : Type} : forall (H1 H2 H3 H4 H5 : hhprop D),
  H1 \-* H4 ==> H5 ->
  (H2 \* ((H1 \* H2 \* H3) \-* H4)) \* H3 ==> H5.
Proof using. introv M. xsimpl. auto. Qed.

Lemma xsimpl_demo_hwand_multiple_2 {D : Type} : forall (H1 H2 H3 H4 H5 : hhprop D),
  (H1 \* H2 \* ((H1 \* H3) \-* (H4 \-* H5))) \* (H2 \-* H3) \* H4 ==> H5.
Proof using. intros. xsimpl. Qed.

Lemma xsimpl_demo_hwand_hempty {D : Type} : forall (H1 H2 H3 : hhprop D),
  (\[] \-* H1) \* H2 ==> H3.
Proof using. intros. xsimpl. Abort.

Lemma xsimpl_demo_hwand_hstar_hempty {D : Type} : forall (H0 H1 H2 H3 : hhprop D),
  ((H0 \* \[]) \-* \[] \-* H1) \* H2 ==> H3.
Proof using. intros. xsimpl. rew_heap. Abort.
(* [xsimpl] does not simplify inner [\[] \-* H1], known limitation. *)

Lemma xsimpl_demo_hwand_iter {D : Type} : forall (H1 H2 H3 H4 H5 : hhprop D),
...
Lemma xsimpl_demo_repr_1 {D : Type} : forall p q (R:int->int->hhprop D),
...
Lemma xsimpl_demo_repr_2 {D : Type} : forall p (R R':int->int->hhprop D),
  R = R' ->
  p ~> R' 3 ==> \exists n, p ~> R n.
Proof using. introv E. xsimpl. subst R'. xsimpl. Qed.

Lemma xsimpl_demo_repr_3 {D : Type} : forall p (R:int->int->hhprop D),
  let R' := R in
  p ~> R' 3 ==> \exists n, p ~> R n.
Proof using.
  intros. dup.
...
Lemma xsimpl_demo_repr_4 {D : Type} : forall p n m (R:int->int->hhprop D),
  n = m + 0 ->
  p ~> R n ==> p ~> R m.
Proof using. intros. xsimpl. math. Qed.

Lemma xsimpl_demo_gc_0 {D : Type} : forall (H1 H2 : hhprop D),
  H1 ==> H2 \* \GC \* \GC.
Proof using. intros. xsimpl. Abort.

Lemma xsimpl_demo_gc_1 {D : Type} : forall (H1 H2 : hhprop D),
  H1 ==> H2 \* \GC \* \Top \* \Top \* \GC.
Proof using.
  intros. dup.
...
Lemma xsimpl_demo_gc_2 {D : Type} : forall (H1 H2 H3 : hhprop D),
...
Lemma xsimpl_demo_gc_3 {D : Type} : forall (H1 H2 : hhprop D),
  H1 \* H2 \* \GC \* \GC ==> H2 \* \GC \* \GC \* \GC.
Proof using. intros. xsimpl. xaffine. Abort.

Lemma xsimpl_demo_gc_4 {D : Type} : forall (H1 H2 : hhprop D),
...
Lemma xchange_demo_1 {D : Type} : forall (H1 H2 H3 H4 H5 H6 : hhprop D),
  H1 ==> H2 \* H3 ->
  H1 \* H4 ==> (H5 \-* H6).
Proof using.
  introv M. dup 3.
...
Lemma xchange_demo_2 {D : Type} : forall A (Q:A->hhprop D) H1 H2 H3,
...
Lemma xchange_demo_4 {D : Type} : forall A (Q1 Q2:A->hhprop D) H,
  Q1 ===> Q2 ->
  Q1 \*+ H ===> Q2 \*+ H.
Proof using. introv M. xchanges M. Qed.

Lemma xsimpl_demo_hfalse {D : Type} : forall (H1 H2 : hhprop D),
  H1 ==> \[False] ->
  H1 \* H2 ==> \[False].
Proof using.
  introv M. dup.
...
Lemma xchange_demo_hforall_l {D : Type} :
 forall (hforall_specialize : forall A (x:A) (J:A->hhprop D), (hforall J) ==> (J x)),
 forall (Q:int->hhprop D) H1,
